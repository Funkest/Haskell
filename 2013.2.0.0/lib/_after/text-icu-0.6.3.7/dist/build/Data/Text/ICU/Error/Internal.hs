...
{-# LINE 37 "Data\Text\ICU\Error\Internal.hsc" #-}

{-# LINE 38 "Data\Text\ICU\Error\Internal.hsc" #-}
...
{-# LINE 83 "Data\Text\ICU\Error\Internal.hsc" #-}
      (offset::Int32) <- (\hsc_ptr -> peekByteOff hsc_ptr 4) ptr
{-# LINE 84 "Data\Text\ICU\Error\Internal.hsc" #-}
...
handleOverflowError guess fill retrieve =
  alloca $ \uerrPtr -> flip fix guess $ \loop n ->
    (either (loop . fromIntegral) return =<<) . allocaArray n $ \ptr -> do
      ...
      case undefined of
        _| err == (15)
{-# LINE 141 "Data\Text\ICU\Error\Internal.hsc" #-}
                     -> return (Left ret)
